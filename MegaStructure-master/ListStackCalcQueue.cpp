#include <iostream>
#include "C:\Users\FouRth\Desktop\Projects in Cpp\MegaHeap\MegaHeap\Chain2.h"
...
void testIterator() {
    ...
    // Нормальный случай: перебор элементов вперед
    {
        auto it = chain.begin();
        assert(*it == 1);
        ++it;
        ...
        assert(*it == 4);
        ++it;
        assert(it == chain.end());  // Достигли конца списка
    }

    // Нормальный случай: перебор элементов назад
    {
        auto it = chain.end();
        --it;
        assert(*it == 4);
        ...
        --it;
        assert(*it == 1);
        assert(it == chain.begin());  // Достигли начала списка
    }

    // Пустая цепь: проверка, что begin() и end() равны
    {
        Chain2<int> emptyChain;
        assert(emptyChain.begin() == emptyChain.end());
    }

    // Попытка инкрементации итератора в конце цепи: не должно приводить к ошибке
    {
        auto it = chain.end();
        ++it;  // Должно оставаться на end(), так как дальше нет элементов
        assert(it == chain.end());
    }

    // Попытка декрементации итератора в начале цепи: не должно приводить к ошибке
    {
        auto it = chain.begin();
        --it;  // Должно остаться в том же положении
        assert(it == chain.begin());
    }
    ...
    // Сравнение итераторов
    {
        ...
        assert(it1 == it2);  // Они указывают на один и тот же элемент

        ++it2;
        assert(it1 != it2);  // Теперь они указывают на разные элементы
    }

    // Попытка работы с цепью после изменения структуры (добавление элемента)
    {
        auto it = chain.begin();
        assert(*it == 1);
        chain.adder.front(0);  // Добавляем элемент в начало
        // Итератор становится недействительным, поведение не определено, тестируем на основе текущей реализации
        it = chain.begin();
        assert(*it == 0);  // Новый элемент стал первым
    }

    // Доступ к элементам через оператор разыменования
    {
        auto it = chain.begin();
        *it = 10;  // Изменяем первый элемент
        assert(*it == 10);
    }
